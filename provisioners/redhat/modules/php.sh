...
# /var/opt/remi/php83/log/php-fpm
# sudo yum list \*-php83-\*
#################
...
if ([ "${4}" == "apache" ] || [ "${4}" == "apache-node" ]); then
    sudo yum install -y php83-php-fpm
    sed -i -e "s#^listen = 127.0.0.1:9000#listen = 127.0.0.1:9830#g" /etc/opt/remi/php83/php-fpm.d/www.conf
...
# set the timezone
sed -i -e "s#\;date\.timezone.*#date.timezone = \"$(catapult company.timezone_redhat)\"#g" /etc/opt/remi/php83/php.ini
# increase the post_max_size
sed -i -e "s#^post_max_size.*#post_max_size = 64M#g" /etc/opt/remi/php83/php.ini
# increase the upload_max_filesize
sed -i -e "s#^upload_max_filesize.*#upload_max_filesize = 16M#g" /etc/opt/remi/php83/php.ini
# hide x-powered-by
sed -i -e "s#^expose_php.*#expose_php = Off#g" /etc/opt/remi/php83/php.ini
# increase php memory limit for tools like composer
sed -i -e "s#^memory_limit.*#memory_limit = 256M#g" /etc/opt/remi/php83/php.ini
# display errors on screen using the default recommendations for development and production
if ([ "$1" = "dev" ] || [ "$1" = "test" ]); then
    sed -i -e "s#^display_errors.*#display_errors = On#g" /etc/opt/remi/php83/php.ini
    sed -i -e "s#^error_reporting.*#error_reporting = E_ALL#g" /etc/opt/remi/php83/php.ini
else
    sed -i -e "s#^display_errors.*#display_errors = Off#g" /etc/opt/remi/php83/php.ini
    sed -i -e "s#^error_reporting.*#error_reporting = E_ALL \& \~E_DEPRECATED \& \~E_STRICT#g" /etc/opt/remi/php83/php.ini
fi

# bundled extensions
# These extensions are bundled with PHP.
sudo yum install -y php83-php-bcmath php83-php-gd php83-php-intl php83-php-mbstring php83-php-opcache php83-php-soap php83-php-xmlrpc
# disable opcache for dev
if [ "$1" = "dev" ]; then
    sudo bash -c 'echo "/var/www" > /etc/opt/remi/php83/php.d/opcache-default.blacklist'
else
    sudo bash -c 'echo "" > /etc/opt/remi/php83/php.d/opcache-default.blacklist'
fi

# external extensions
# These extensions are bundled with PHP but in order to compile them, external libraries will be needed.
sudo yum install -y php83-php-gmp php83-php-mysqlnd
...
sudo yum install -y php83-php-pecl-imagick-im6 php83-php-pecl-uploadprogress
...
if ([ "${4}" == "apache" ] || [ "${4}" == "apache-node" ]); then
    ...
    # rh-php83
    rm --force "/opt/rh/rh-php83/root/usr/lib64/php/modules/newrelic.so"
    cp "/usr/share/newrelic/newrelic-php5-8.4.0.231-linux/agent/x64/newrelic-20170718.so" "/opt/remi/php83/root/usr/lib64/php/modules/newrelic.so"
    cp "/usr/share/newrelic/newrelic-php5-8.4.0.231-linux/scripts/newrelic.ini.template" "/etc/opt/remi/php83/php.d/newrelic.ini"
    # rh-php73
    ...
    NR_INSTALL_PHPLIST="/usr/bin:/opt/rh/rh-php71/root/usr/bin:/opt/rh/rh-php72/root/usr/bin:/opt/rh/rh-php73/root/usr/bin:/opt/remi/php83/root/usr/bin"; export NR_INSTALL_PHPLIST
    ...
    # cat /var/opt/remi/php83/log/php-fpm/error.log* | grep max
    ...
    sed -i -e "s#^pm.max_children.*#pm.max_children = 300#g" /etc/opt/remi/php83/php-fpm.d/www.conf
    ...
    sed -i -e "s#.*pm.max_requests.*#pm.max_requests = 500#g" /etc/opt/remi/php83/php-fpm.d/www.conf
    ...
    /opt/remi/php83/root/usr/bin/php --version
    /opt/remi/php83/root/usr/bin/php --modules
